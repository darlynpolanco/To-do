...
// Configuración de la API - usa tu URL local
const API_BASE_URL = "https://localhost:7089";

export default function AuthForm3() {
  ...
  // Manejar envío de link de recuperación
    ...
    if (isLoggedIn) {
      return (
        <div className="text-center py-10">
          ...
          <h3 className="text-xl font-semibold text-gray-800 mb-2">¡Sesión iniciada!</h3>
          <p className="text-gray-600 mb-4">Bienvenido a tu cuenta</p>
          <button
            onClick={handleLogout}
            className="bg-blue-500 hover:bg-blue-600 text-white rounded-full px-6 py-2 font-semibold transition"
          >
            Cerrar sesión
          </button>
        </div>
      ...
      <div className="bg-white w-full max-w-6xl h-auto md:h-[520px] rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden relative">
        ...
        <div className="w-full md:w-1/2 h-full p-10 relative overflow-hidden bg-gray-100">
          <AnimatePresence mode="wait" initial={false}>
            <motion.div
              key={authMode + isLoggedIn.toString()}
              variants={{
                initial: (direction) => ({
        ...
        {/* Panel lateral (solo visible cuando no hay sesión) */}
        {!isLoggedIn && (
          <motion.div
            key={`panel-${authMode}`}
